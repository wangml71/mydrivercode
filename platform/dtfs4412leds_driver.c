...
void led_init(struct fs4412leds_driver_dev *pmydev, struct device_node *pnode)
{
	int lednum = 0;
...
void led_free(struct fs4412leds_driver_dev *pmydev)
{
	int lednum = 0;
	for(lednum=0; lednum<4; lednum++)
	{
...
struct platform_driver fs4412leds_driver = {
	.driver = {.name = "fs4412led", .of_match_table = myleddrv_of_ids,},
